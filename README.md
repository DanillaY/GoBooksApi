...
*	🟢 /getSameBookInDifferentStores [<span style="color:rgba(9,234,87, 1);">*GET*</span>] - эндпоинт, который отвечает за получение массива книг с более подробным поиском, принимаемые параметры: search, category, author.
...
- bookInfoType - параметр, отвечающий за полноту отправляемых данных, изначальное значение bookInfoType равно partial, если значение равно стандартному, тогда только данные из полей id, title, author, category, current_price, old_price, img_path, page_book_path, vendor, age_restriction будут загружены и отправлены, иначе все поля будут отправлены.
...
##### Query параметры для метода getSameBookInDifferentStores

Данный метод возвращает массив книг в теге books, основан на параметрах запроса серч, принимает параметры search, category, author.

Пример запроса с применением различных query параметров: <http://172.21.80.1:8089/getSameBookInDifferentStores?search=пушкин&category=Русская драматургия>

##### Query параметры для метода addNewBookSubscriber 

Для добавления нового пользователя эндпоинт принимает 2 параметра, а именно bookId, который обозначает ID книги, на которую вы хотите подписать пользователя и userEmail, обозначающий почту пользователя, на которое должно прийти оповещение.
